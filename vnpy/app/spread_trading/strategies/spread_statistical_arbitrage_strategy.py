...
class SpreadStatisticalArbitrageStrategy(SpreadStrategyTemplate):
    """
    差价开仓与boll 上下轨开仓结合
    """
    ...
    parameters = [
        ...
        "payup",
        "interval",
        "buy_price", "sell_price", "cover_price", "short_price"
    ...
    def on_spread_bar(self, bar: BarData):
        ...
        if not self.spread_pos:
            if bar.close_price >= self.boll_up and bar.close_price >= 150:
            ...
            elif bar.close_price <= self.boll_down and bar.close_price <=50 :
            ...
            if bar.close_price <= self.boll_mid and bar.close_price <= 50:
                self.start_long_algo(
                    bar.close_price + 10,
                    abs(self.spread_pos),
                    payup=self.payup,
            ...
            if bar.close_price >= self.boll_mid and bar.close_price >= 150:
                self.start_short_algo(
                    bar.close_price - 10,
                    abs(self.spread_pos),
                    payup=self.payup,
