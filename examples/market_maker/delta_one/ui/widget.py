...
class QuotingManager(QtWidgets.QWidget):
    """"""
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine):
        ...
        self.quoting_engine: QuotingEngine = main_engine.get_engine(APP_NAME)

        self.managers = {}

        self.init_ui()
        self.register_event()
        self.quoting_engine.init_engine()
        self.update_class_combo()

    def init_ui(self):
        """"""
        self.setWindowTitle("DeltaOne做市")
    ...
    def update_class_combo(self):
        ...
        self.class_combo.addItems(
            self.quoting_engine.get_all_strategy_class_names()
        )
    ...
    def register_event(self):
        ...
        self.event_engine.register(
            EVENT_DO_STRATEGY, self.signal_strategy.emit
        )
    ...
    def process_strategy_event(self, event):
        ...
        if strategy_name in self.managers:
            ...
            manager = StrategyManager(self, self.quoting_engine, data)
    ...
    def add_strategy(self):
        ...
        parameters = self.quoting_engine.get_strategy_class_parameters(class_name)
        ...
        if n == editor.Accepted:
            ...
            self.quoting_engine.add_strategy(
                class_name, strategy_name, setting
            )
            self.quoting_engine.save_strategy_setting()
...
class StrategyManager(QtWidgets.QFrame):
    ...
    def __init__(
        self, quoting_manager: QuotingManager, quoting_engine: QuotingEngine, data: dict
    ):
        """"""
        super(StrategyManager, self).__init__()

        self.quoting_manager = quoting_manager
    ...
    def init_strategy(self):
        """"""
        self.quoting_engine.init_strategy(self.strategy_name)

    def start_strategy(self):
        """"""
        self.quoting_engine.start_strategy(self.strategy_name)

    def stop_strategy(self):
        """"""
        self.quoting_engine.stop_strategy(self.strategy_name)

    def edit_strategy(self):
        """"""
        strategy_name = self._data["strategy_name"]

        parameters = self.quoting_engine.get_strategy_parameters(strategy_name)
        editor = SettingEditor(parameters, strategy_name=strategy_name)
        n = editor.exec_()

        if n == editor.Accepted:
            setting = editor.get_setting()
            self.quoting_engine.edit_strategy(strategy_name, setting)

    def remove_strategy(self):
        """"""
        result = self.quoting_engine.remove_strategy(self.strategy_name)

        # Only remove strategy gui manager if it has been removed from engine
        if result:
            self.quoting_engine.save_strategy_setting()
            self.quoting_manager.remove_strategy(self.strategy_name)
...
class SettingEditor(QtWidgets.QDialog):
    ...
    def init_ui(self):
        ...
        # Add name edit if add new strategy
        if self.class_name:
